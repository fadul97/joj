...
{
    D3D11Canvas* m_canvas = new D3D11Canvas(580, 10, 200, 580, joj::Color(0.1f, 0.1f, 0.1f, 1.0f));
    JOJ_LOG_IF_FAIL(m_canvas->create(device));
    m_canvas->set_hovered_color(joj::Color(0.23f, 0.23f, 0.23f, 1.0f));

    D3D11Button* m_button = new D3D11Button(600, 200, 100, 100, joj::Color(0.0f, 1.0f, 0.0f, 1.0f));
...
void joj::D3D11GUI::update(const f32 dt, const i32 xmouse, const i32 ymouse, const b8 clicked)
{
    ...
    {
        m_widgets[hovered_index]->update();
    }
